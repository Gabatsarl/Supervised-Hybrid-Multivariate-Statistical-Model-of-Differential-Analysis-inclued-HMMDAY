...
RESULT<-simulation_NB(B=300, # nombre de simulation,
                      ...
                      groups=groups)
save(list = ls(), file ="/Users/fatimaakpo/Downloads/Student/simul_no_data_Small_Scenario_ST1.2.RData")


## Load the data

load("/Users/fatimaakpo/Downloads/Student/simul_no_data_Small_Scenario_ST1.2.RData")
...
ggsave("/Users/fatimaakpo/Downloads/simul1_nodata_ST1.2.pdf",plot=gplot1.2,
...
gplot1.2_MSE <- ggplot(data_long, aes(x = Var2, y = value, fill = Var2)) + 
  geom_boxplot(outlier.shape = NA) +
  labs(title = expression("" * tau * " = 0.03, " * n/p * " = 1%"), 
       x = "", 
       y = "RMSE") +
  ...
  scale_fill_discrete(guide = "none") +
  theme(plot.title = element_text(hjust = 0.5))+  # Centrer le titre
  coord_cartesian(ylim = c(0, 12000))  # Fixer les limites de l'axe y


gplot1.2_MSE


ggsave("/Users/fatimaakpo/Downloads/RMSE_scenario_ST1.2.pdf",plot=gplot1.2_MSE,
...
gplot1.2_R2 <- ggplot(data_long, aes(x = Var2, y = value, fill = Var2)) + 
  geom_boxplot(outlier.shape = NA) +
  labs(title = expression("" * tau * " = 0.03, " * n/p * " = 1%"), 
       x = "", 
       y = "R2") +
...
ggsave("/Users/fatimaakpo/Downloads/R2_scenario_ST1.2.pdf",plot=gplot1.2_R2,
       width = 4, height = 2.8, units = "in", dpi = 300,bg = "white")

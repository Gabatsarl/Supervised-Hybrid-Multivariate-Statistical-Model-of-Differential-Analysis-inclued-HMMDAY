...
myqqplot2 = function(pval,seuil){
  ...
  return(plot1)
  #df1 = data.frame(y=c(pval,exp(-log(10)* expect)), pvalue=c(rep('Observed',length(pval)),
  #                                                       rep('Expected',length(expect))))
  #plot2 <- gghistogram(df1, x = "y", fill = "pvalue",
  #                    xlab="pvalue",ylab="Frequency")
  #plot2 <- gghistogram(df1, x = "y", palette = c("red", "blue"),
  #                    fill = "pvalue",color = "pvalue",xlab = "pvalue", ylab = "Frequency") 

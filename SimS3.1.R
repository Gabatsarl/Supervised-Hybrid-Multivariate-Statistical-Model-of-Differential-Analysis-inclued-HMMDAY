...
simulation_NB<-function(B, # Number of replication,
  ...
  for (k in 1:B){
    ...
    eps<-rt(n_samples, df= 5) #rnorm(n_samples,0,1)
...
save(list = ls(), file ="/Users/fatimaakpo/Downloads/simul_no_data_Small_Scenario_ST3.1.RData")



#boxplot(RESULT$LAMBDA[,1])    
load("/Users/fatimaakpo/Downloads/Student/simul_no_data_Small_Scenario_ST3.1.RData")
...
gplot3.1 <- ggplot(data_long, aes(x = Var2, y = value, fill = Var2)) + 
  geom_boxplot(outlier.shape = NA) +
  labs(title = expression("" * tau * " = 0.01, " * n/p * " = 10%"), 
       x = "", 
       y = "Impact Factor") +
  theme_pubr() +
...
ggsave("/Users/fatimaakpo/Downloads/simul_no_data_Small_Scenario_ST3.1.pdf",plot=gplot3.1,
...
## Create a sample matrix similar to the one in the image
#data_qqplot <- Matrice_data1<-matrix(0,ncol = 9, nrow = Inf)
## Set row and column names

#data_qqplot<-cbind(pval1,pval2,pval3,pval4,pval5,pval7,pval8,pval9,pval10)
#colnames(data_qqplot) <- c(
#  "ALDEx2","Corncob", "dearseq", "DESeq2", "dream", "edgeR",
#  "kimma", "limma", "metagenomeSeq"
#)
library(tidyr)
library(dplyr)
...
print(result_plot)
ggsave("/Users/fatimaakpo/Downloads/qqplot_scenario_ST3.1.pdf",plot=result_plot,
       width = 4.2, height = 3, units = "in", dpi = 300,bg = "white")
...
gplot3.1_MSE <- ggplot(data_long, aes(x = Var2, y = value, fill = Var2)) + 
  geom_boxplot(outlier.shape = NA) +
  labs(title = expression("" * tau * " = 0.01, " * n/p * " = 10%"), 
       x = "", 
       y = "RMSE") +
  ...
  scale_fill_discrete(guide = "none") +
  theme(plot.title = element_text(hjust = 0.5))+  # Centrer le titre
  coord_cartesian(ylim = c(0, 2000))  # Fixer les limites de l'axe y


gplot3.1_MSE


ggsave("/Users/fatimaakpo/Downloads/RMSE_scenario_ST3.1.pdf",plot=gplot3.1_MSE,
       width = 4.2, height = 3, units = "in", dpi = 300,bg = "white")
...
gplot3.1_R2 <- ggplot(data_long, aes(x = Var2, y = value, fill = Var2)) + 
  geom_boxplot(outlier.shape = NA) +
  labs(title = expression("" * tau * " = 0.01, " * n/p * " = 10%"), 
       x = "", 
       y = "R2") +
  ...
  scale_fill_discrete(guide = "none") +
  theme(plot.title = element_text(hjust = 0.5))+  # Centrer le titre
  coord_cartesian(ylim = c(0, 0.1))  # Fixer les limites de l'axe y


gplot3.1_R2

ggsave("/Users/fatimaakpo/Downloads/R2_scenario_ST3.1.pdf",plot=gplot3.1_R2,
       width = 4.2, height = 3, units = "in", dpi = 300,bg = "white")
